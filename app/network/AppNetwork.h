...
public:
    ...
    virtual void login(const string & name, const string & password, const function<void(const MyAccount &)> & callback) = 0;
    virtual void registration(const MyAccount & acc, const function<void(const int &)>& callback) = 0; //если id == 0, то неудачно
    ...
    virtual void sendMessage(const Message & msg, const function<void(const bool &)> & callback) = 0; // отправилось или нет bool
    virtual void getUser(const int & id, const function<void(const UserInfo &)> & callback) = 0;
    virtual void saveMyAvatar(const string &path, const function<void(const string &)> & callback) = 0;
    virtual void getMessages(const int & start, const int & begin, const int & amount, const function<void(vector<Message> &)> & callback) = 0;
    virtual void addChat(ChatRoom & room, const function<void(const ChatRoom &)> & callback) = 0;
